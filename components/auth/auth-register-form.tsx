...
const RegisterForm = () => {
  ...
  const submitHandler = (values: z.infer<typeof RegisterSchema>) => {
    ...
    setSuccess("")
    startTransition(() => { 
      register(values).then(data => data.success ? setSuccess(data.success) : setError(data.error))
    })
  ...
  >
    <Form {...form}>
      ...
      >
        ...
        <Button type='submit' disabled={isPending} className='w-full mt-5'>Register</Button>
      </form>
    </Form>
  </Cardwrapper>
)
