...
export const setNewPassword = async (
  values: z.infer<typeof NewResetPasswordSchema>,
  token: string
) => {
  ...
  const NewResetPasswordSchemaPasswordToken =
    await getResetPasswordTokenByToken(token);
  const existingUser = await getUserByEmail(
    NewResetPasswordSchemaPasswordToken.email
  );
  if (!existingUser) {
