...
const verficationToken = async (token: string) => {
  const currentVerificationTokenBearer =
    await getVerificatinTokenByToken(token);
  const existingUser = await getUserByEmail(
    currentVerificationTokenBearer?.email
  );
  ...
  if (new Date(currentVerificationTokenBearer.expires) <= new Date()) {
    await db.confirmationToken.delete({
      where: { token, id: currentVerificationTokenBearer?.id },
    });
